...
- 为箱子、告示牌、悬挂告示牌、旗帜、床、潜影盒、头颅、装饰陶罐和传送门等特殊方块提供轻量几何预览
...
- 告示牌文字、旗帜图案、容器内容和头颅皮肤等依赖方块实体 NBT 的细节暂不显示，但对应方块的几何形状和朝向可正常预览。
...
- `src/viewer/special-block-models.ts`：方块实体渲染类型的轻量几何模型与朝向映射
